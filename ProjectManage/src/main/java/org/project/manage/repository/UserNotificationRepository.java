...
public interface UserNotificationRepository extends JpaRepository<UserNotificationEntity, Long> {
	...
	@Query(value = "select a.* from user_notification a where CONVERT(datetime, a.created_date,11)>= DATEADD(day,-7, GETDATE()) AND a.user_id = ?1 and a.delete_date is null order by a.id desc", nativeQuery = true)
